...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
        ...
        <html>
            <body>
                ...
                <ol>
                    ...
                    <br/>
                    <xsl:apply-templates select="." mode="szczegolyZespol"/>
                </ol>
            </body>
        </html>
    </xsl:template>
    ...
    <xsl:template match="ROW" mode="szczegolyZespol">
        <div>
            <strong id="{ID_ZESP}">NAZWA:  <xsl:value-of select="NAZWA"/> </strong>
            <br/>
            <strong>ADRES: <xsl:value-of select="ADRES"/> </strong>
            <br/> <br/>
            <xsl:variable name="liczbaPrac" select="count(PRACOWNICY/ROW[ID_ZESP = current()/ID_ZESP])"/>
            <xsl:if test="$liczbaPrac > 0">
                <table border="1">
                    <tr>
                        <th>Nazwisko</th>
                        <th>Etat</th>
    ...
    <xsl:template match="ROW/PRACOWNICY/ROW" mode="pracownik">
        <tr>
